...
function setup(data: Partial<CrucibleConfirmOptions>) {
  ...
  const fixture: ComponentFixture<CrucibleConfirmDialogComponent> =
    TestBed.createComponent(CrucibleConfirmDialogComponent);
...
describe('CrucibleConfirmDialogComponent', () => {
  it('renders the title and message', () => {
    ...
    expect(el.querySelector('h2[mat-dialog-title]')?.textContent).toContain(
      'Delete Project?',
    );
  ...
  it('has no positive tabindex', () => {
    ...
    expect(
      Array.from(withTab).every(
        (n: any) => Number(n.getAttribute('tabindex')) <= 0,
      ),
    ).toBeTrue();
  });
